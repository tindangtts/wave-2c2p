...
export default function NewRecipientPage() {
  ...
  async function onSubmit(data: RecipientFormInput) {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        // Handle Zod field errors from API
        if (err.details?.issues) {
          ...
          toast.error(err.error || "Failed to create recipient");
        }
        return;
      }
  ...
  return (
    <>
      <BackHeader title={t("title_new_recipient")} />
                ...
                <SelectContent>
                  {occupationOptions.map((opt) => (
                    <SelectItem key={opt} value={opt}>
                      {t(
                        `occupation_${opt}` as Parameters<typeof t>[0]
                      )}
                    </SelectItem>
                  ))}
                </SelectContent>
